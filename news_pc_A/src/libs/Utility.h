...
static var_vd GetTagValue(var_1 *xml, var_4 xml_len, const var_1 *tag_name, var_1 *tag_value, var_1 if_cdata)
{
	var_1 *sPos = 0, *ePos = 0;
	var_1 startTag[32] = {0};
	var_1 endTag[32]   = {0};
	...
	{
		snprintf(startTag, 32, "%s%s%s", "<", tag_name, "><![CDATA[");
		snprintf(endTag, 32, "%s%s%s", "]]></", tag_name, ">");
	}
	...
	if (sPos != NULL && sPos - xml < xml_len)
	{
		...
		{
			len = ePos - sPos - strlen(startTag);
			strncpy(tag_value, sPos + strlen(startTag), len);
		}
	}
	tag_value[len] = 0x0;
	return;
