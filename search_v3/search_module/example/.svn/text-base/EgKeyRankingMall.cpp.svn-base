...
#ifdef DEBUG
	printf("%s, dict_size()=%d\n", file.c_str(), dict.size());
#endif
	return true;
}

bool CEgKeyRanking::InitMall(SSearchDataInfo* psdi, const string& strConf)
{

	/*FID_BRAND_NAME = GetFieldId("brand_name");
	if (FID_BRAND_NAME == -1)
	{
		printf("brand_name field does not exist\n");
		return false;
	}*/
#ifdef DEBUG
	cout << "module_path = " << m_strModulePath << endl;
#endif
	//加载词典
	...
	{
		string query = "手机";
		HASHVECTOR cate_vec = m_key2Cate[query];
		HASHVECTOR pid_vec = m_key2Pid[query];
		cout << "query=" << query << endl;
		cout << "cate list:" << endl;
		for (int i=0; i<cate_vec.count; i++) {
			u64 cate_id = 0;
			int weight = 0;
			...
			cout << "cate_id=" << cate_id << " cate_path=" << cate_path <<" weight=" << weight << endl;
		}
		cout << "pid list:" << endl;
		for (int i=0; i<pid_vec.count; i++) {
			int pid = 0;
			memcpy(&pid, pid_vec.data+i*pid_vec.size, 4);
			cout << "pid=" << pid << endl;
		}
	}
#endif
...
#ifdef DEBUG
	{
		int pid_test = 1074267602;
		HASHVECTOR core_word_test=m_pid2Core[pid_test];
		cout << "test_pid=" << pid_test << endl;
		cout << "test_core_word=";
		for (int i=0; i<core_word_test.count; i++) {
			char word[20];
			memcpy(word, core_word_test.data+i*core_word_test.size, core_word_test.size);
			cout << word << " ";
		}
		cout << endl;
	}
...
#ifdef DEBUG
	{
		...
		cout << "query=" << query << endl;
		cout << "mall_count=" << rate.first <<" pub_count=" << rate.second << endl;
	}
#endif
	...
	TITLE_FID = GetFieldId("name");
	if (TITLE_FID == -1) {
		COMMON_LOG(SL_ERROR, "title field does not exist");
		return false;
	} else {
		m_fid2fi[TITLE_FID] = TITLENAME;
	}
	COMMON_LOG(SL_INFO, "title field ok");

	TITLESYN_FID = GetFieldId("titlesyn");
	if (TITLESYN_FID == -1) {
		COMMON_LOG(SL_ERROR, "titlesyn field does not exist");
		return false;
	} else {
		m_fid2fi[TITLESYN_FID] = TITLESYN;
	}
	COMMON_LOG(SL_INFO, "titlesyn field ok");
	...
	BOTTOMCATE_FID = GetFieldId("lowest_cat");
	if (BOTTOMCATE_FID == -1) {
		COMMON_LOG(SL_ERROR, "lowest_cat field does not exist");
		return false;
	} else {
		m_fid2fi[BOTTOMCATE_FID] = BOTTOMCATE;
	}
	COMMON_LOG(SL_INFO, "lowest_cat field ok");
	...
	INPUT_DATE_FID = GetFieldId("input_date");
	if (INPUT_DATE_FID == -1) {
		COMMON_LOG(SL_ERROR, "input_date field does not exist");
		return false;
	}
	COMMON_LOG(SL_INFO, "input_date field ok");

	COMMON_LOG(SL_INFO, "init mall: success");
	return true;
}

void CEgKeyRanking::QueryAnalyseMall(CDDAnalysisData* pa, SQueryClause& qc)
{
#ifdef DEBUG
	printf("enter QueryAnalyseMall\n");
#endif
	//初始化pa
...
#ifdef DEBUG
	cerr << "\n\n>>>>>>>>>>>>>>>>>>>>>> query = " << pa->m_mallAnalysisPart.querystr << endl;
#endif
	...
	{	//产品搜索/品牌+产品搜索
		...
		pa->m_queryFieldIds.push_back(GetFieldId("subname"));
		pa->m_queryFieldIds.push_back(GetFieldId("titlesyn"));
		pa->m_queryFieldIds.push_back(GetFieldId("brand_name"));
		pa->m_queryFieldIds.push_back(GetFieldId("lowest_cat"));
		pa->m_queryFieldIds.push_back(GetFieldId("other_cat"));
...
#ifdef DEBUG
	cout << ">>>>>>>>>>>>>m_mallAnalysisPart.ifBrdQuery = " << pa->m_mallAnalysisPart.ifBrdQuery << endl;
#endif
}
...
void CEgKeyRanking::FilterMallResults(vector<SResult>& vRt) {
...
#ifdef DEBUG
	cout << "before filter: result.size() = " << vRt.size() << endl;
#endif
	vRt.erase(std::remove_if(vRt.begin(), vRt.end(), filter_zero_weight), vRt.end());
#ifdef DEBUG
	cout << "after filter: result.size() = " << vRt.size() << endl;
#endif
}

void CEgKeyRanking::CombineComputeWeight_ComQ(
	CDDAnalysisData* pa,
	vector<SResult>& vRt)
{	//一般搜索
	...
	{	//可做精简
		...
		for (size_t i = 0; i < vRt.size(); ++i)
		{
			...
			{	//不相关嫌疑
				bool ifRel = false;
				//int pid = m_refDocReader->GetDataFieldValue<int>(rt[i].docID, PRODUCTID_FID);
				...
				//		if (RelCate.find(lower->second[0]) != RelCate.end())
...
void CEgKeyRanking::CombineComputeWeight_PrdQ(
	CDDAnalysisData* pa,
	vector<SResult>& vRt)
{	//含品类搜索
#ifdef DEBUG
	cout << ">>>>>>>>>>>>>>>CombineComputeWeight_PrdQ" << endl;
#endif
	//修改BaseRel, 为精简准备
	...
	{	//可做精简
...
#ifdef DEBUG
		cout << "m_mallAnalysisPart.relCnt_fb = " << pa->m_mallAnalysisPart.relCnt_fb << endl;
#endif
		if (false == RelCate.empty())
		{
			for (size_t i = 0; i < vRt.size(); ++i)
			{
				...
				//cout << "relScr = " << relScr << endl;
				if ( (true == RelType && relScr != 2)		//不匹配反馈
					|| (false == RelType && relScr < 1) )	//不匹配反馈或产品中心
				{	//不相关嫌疑
					//cout << "RelType = " << RelType << endl;
					bool ifRel = false;
					//int pid = m_refDocReader->GetDataFieldValue<int>(out[i].docID, PRODUCTID_FID);
					//multimap<int, vector<int> >::const_iterator lower = pid2cate.find(pid);
					int docID = vRt[i].nDocId;
					...
					//	}
					//}
					//cout << "ifRel = " << ifRel << endl;
					//if (false == ifRel && out[i].weight > BaseRelFactor)
...
void CEgKeyRanking::CombineComputeWeight_BrdQ(
	CDDAnalysisData* pa,
	vector<SResult>& vRt)
{	//品牌搜索
...
void CEgKeyRanking::SingleRank(CDDAnalysisData* pa, SMatchElement& me, SResult& rt) {
	...
	int pdtCoreWeight = 0;
	{
		//cout << "start to ComputeCommerceWeight..." << endl;
...
void CEgKeyRanking::MultiRank(CDDAnalysisData* pa, SMatchElement& me, SResult& rt) {
	...
	bool ifDisMatch = true;						//词距是否匹配（标题）
	{
		...
		vector<vector<int> > vPosTi(querySz);	//标题出词位置记录
		{
			...
			{	//for one term
				//ivtAddr = vKey[k].invertAddr;
				//ivtCnt = vKey[k].invertCount;
				added_T = added_ti = added_syn = added_b = added_u = false;
				//int cnt = pa->m_mallAnalysisPart.keysRepeat[k] & 0xff;	//取后8位
...
void CEgKeyRanking::ComputeWeightMall(CDDAnalysisData* pa, SMatchElement& me, SResult& rt) {
	rt.nDocId = me.id;
...
void CEgKeyRanking::ComputeCommerceWeight(const int docID,
		int& commerceScr, int& saleDetail) const { //商业评估函数
	int sale = m_funcFrstInt(m_saleWeekProfile, docID);
	int date = m_funcFrstInt(m_inputDateProfile, docID); //@todo:这里date表示上架时间距离1970年1月1日的秒数
	if (date > 0){                                       //而实际是要计算出到现在的天数
		int pid = m_funcFrstInt64(m_isPidProfile, docID);
#ifdef DEBUG
		cout << "pid = " << pid << "  data = " << date << endl;
#endif
	}
	if (sale) {
		if (sale < 10) {
			...
			commerceScr = sale < 30 ? 2 : 3;
			saleDetail = sale / 10;
		} else {
			commerceScr = sale < 300 ? 4 : 5;
			saleDetail = 10 + sale / 100;
		}
	}
...
void CEgKeyRanking::BrandQRank(CDDAnalysisData* pa, SMatchElement& me, SResult& rt) const
{	//品牌搜索相关度计算
	//基本信息
	int docID = rt.nDocId;

	//字段信息
	int fieldScr = 0;		//字段得分[0-3]
	bool ifInTitle = false;						//是否出在标题
	bool ifInSyn = false;							//是否出在同义扩展
	bool ifInBrand = false;						//是否出在品牌
	{
		//Invert* ivtAddr;
		int ivtCnt, cnt;
		int fid, fieldNum;
		...
		fieldScr = ifInBrand ? 3 : (ifInTitle ? 2 : (ifInSyn ? 1 : 0));
	}
	...
	//int sale = m_refDocReader->GetDataFieldValue<int>(docID, SALE_WEEK_FID);
	int sale = m_funcFrstInt(m_saleWeekProfile, docID);
	/*{
		//int date = m_refDocReader->GetDataFieldValue<int>(docID, INPUT_DATE_FID);
	...
	if (ifInBrand) //对品牌字段中的所有商品处理
	{
		...
		//int pid = m_refDocReader->GetDataFieldValue<int>(docID, PRODUCTID_FID);
		//multimap<int, vector<int> >::const_iterator lower = pid2cate.find(pid);
		//if (lower != pid2cate.end())
		//{
		//	multimap<int, vector<int> >::const_iterator upper =
	...
	rt.nScore += FakeScore  //1101000000
							...
							+ fieldScr * (Weight_Field+Weight_Brand+Weight_Dis) //60+30+30
...
void CEgKeyRanking::ComputeSpecialWeight(
	const int docID,
	CDDAnalysisData* pa,
	int& fbCateWeight,
	int& fbPidWeight,
	...
	int& relScr)
{
	...
	if (pid == 1206913401) {
		...
		cout << " size=" << m_pid2Core[pid].count << endl;
	}
#endif
	size_t nCnt = 0;
	u64* ptr = (u64*)m_funcValPtr(m_clsProfile, docID, nCnt); //返回类别指针
...
void CEgKeyRanking::ReRankingMall(vector<SResult>& vRes, CDDAnalysisData* pa)
{
#ifdef DEBUG
...
void CEgKeyRanking::SortMall(vector<SResult>& vRes, int from, int to, CDDAnalysisData* pa)
{
	// do somethine more 
