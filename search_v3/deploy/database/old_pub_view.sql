...
DROP VIEW IF exists test_search_tmp3;

/*==============================================================*/
/* VIEW: test_search                                            */
/*==============================================================*/
/* TODO: index view 利用索引视图加速 */
CREATE VIEW test_search_tmp3 AS
SELECT
    /* ================ product_core_search ===================*/
    /* 商品ID 主键 */
...
WHERE
    ...
    AND (
        ( core.product_type <> 99 AND core.product_type <> 1 )
        OR isnull(core.product_type) 
        OR core.product_type = ''
    ) 
    AND (
        isnull(core.shop_id) 
