		...
		pair<int, vector<SGroupByInfo> >& prGpInfo)
{
    ...
    hash_map<string, string>::iterator itCatPaths = pad->m_hmUrlPara.find(MY_CAT_CUSTOM_GROUP);
    if(itCatPaths != pad->m_hmUrlPara.end())
    {
        unsigned long long cat_id=0;
        cat_id = TranseClsPath2ID(itCatPaths->second.c_str(),itCatPaths->second.length());
	    if (gfNode.nId==CAT_CUSTOM_GROUP)
	    {
		    m_funcGpClassPtr(m_vProfiles[gfNode.nField],vDocIds,1000,prGpInfo.second,false, cat_id);
		    prGpInfo.first=gfNode.nField;
	    }
    }
...
{
	SESSION_LOG(SL_INFO,"query rewrite !!");
	...
	/*hash_map<string, string>::iterator itProductFlag = hmParams.find(PRODUCT_FLAG);
    if (itProductFlag != hmParams.end())
    {
        hmParams.insert(make_pair(FILT_PRODUCT_FLAG,itProductFlag->second));
        hmParams.erase(itProductFlag);
    }*/
}
...
void CQueryResultPack::SetGroupByAndFilterSequence(IAnalysisData* pad, vector<SFGNode>& vFgn)
{
    ...
    hash_map<string, string>::iterator itScore = pad->m_hmUrlPara.find(FILT_CUSTOM_SCORE);
    if (itScore != pad->m_hmUrlPara.end()) {
        SFGNode s;
        s.nCid = -1;
        s.nType = SFGNode::FILT_TYPE;
        s.nId = CUSTOM_SCORE_FILT;
        vFgn.push_back(s);
    }
    hash_map<string, string>::iterator itProductFlag = pad->m_hmUrlPara.find(FILT_PRODUCT_BIT);
    if (itProductFlag != pad->m_hmUrlPara.end())
    {
        SFGNode s;
        s.nCid = -1;
        s.nType = SFGNode::FILT_TYPE;
        s.nId = PRODUCT_BIT_FILT;
        vFgn.push_back(s);
    }
    hash_map<string, string>::iterator itDis = pad->m_hmUrlPara.find(FILT_CUSTOM_DISCOUNT);
    if (itDis != pad->m_hmUrlPara.end())
    {
        SFGNode s;
        s.nCid = -1;
        s.nType = SFGNode::FILT_TYPE;
...
void CQueryResultPack::CustomFilt(IAnalysisData* pad, vector<int>& vDocIds, SFGNode& fgNode)
{
    ...
    if (itProductFlag != pad->m_hmUrlPara.end() )
    {
        iFlag = atoi(itProductFlag->second.c_str());
        for (size_t i = 0; i< vDocIds.size(); i++)
        {
            //cout<<iFlag<<endl;
            if (vDocIds[i] == -1)
            {
            ...
            int iProductFlag = m_funcFrstInt64(m_ProductBit, vDocIds[i]);
            if (!(iProductFlag & (1<<(iFlag-1))))
            {
                vDocIds[i] = -1;
            }
        //cout<<vDocIds[i]<<endl;
        }
        //cout<<vDocIds.size()<<endl;
	...
	float iDiscount = 0;
	int myDis = 0;
	if(fgNode.nId == FILT_DISCOUNT)
	{
		//parse discount param	
		hash_map<string,string>::iterator p = pad->m_hmUrlPara.find(FILT_CUSTOM_DISCOUNT);	
		string szDiscount;	
		string temp;
		CCustPriceFilt CustFilt;
		if(p != pad->m_hmUrlPara.end())
		{
			//1.get discoutn param	
			szDiscount = p->second; 
			temp = szDiscount;
			CustFilt.SignalParse(szDiscount);
			int dd_price = 0;
			//2.filt docid 	
			for(int k = 0;k<iDocSize;k++)
			{
				if(temp == "")
						break;
				dd_price = m_funcFrstInt(m_PurPriceProfile,vDocIds[k]);
				if(vDocIds[k]==-1)
					continue;
				GetRealPrice(RealPrice,vDocIds[k],m_PriceProfile);	
				iDiscount = (float)RealPrice/dd_price ;
				myDis = (int)(iDiscount*100 + 0.5);
				for(int cVal = 0;cVal<CustFilt.m_vSnglVal.size();cVal++)
				{
					if(myDis != CustFilt.m_vSnglVal[cVal])
					{
						vDocIds[k]=-1;
						break;
					}
				...
				for(int cRange = 0; cRange <CustFilt.m_vPrVal.size();cRange++)
				{
					...
					if(myDis<=CustFilt.m_vPrVal[cRange].first 
							|| myDis>CustFilt.m_vPrVal[cRange].second)
					{
						vDocIds[k]=-1;
						break;
					}
    ...
    hash_map<string, string>::iterator itScore = pad->m_hmUrlPara.find(FILT_CUSTOM_SCORE);
    pProfile = FindProfileByName("score");
    if (itScore != pad->m_hmUrlPara.end() && itScore->second != "")
    {
        vector<string> vec;
        SplitToVecEx(itScore->second.c_str(), vec, "~");
        if (vec.size() == 2)
        {
            filt_score[0] = atoi(vec[0].c_str());
            filt_score[1] = atoi(vec[1].c_str());
        }
        else if (vec.size() == 1)
        {
            filt_score[0] = atoi(vec[0].c_str());
        ...
        for (size_t i = 0; i< vDocIds.size(); i++)
        {
            ...
            if (fgNode.nId == CUSTOM_SCORE_FILT)
            {
                iScore = m_funcFrstInt(pProfile, vDocIds[i]);
                if (!(iScore >= filt_score[0] && (iScore <= filt_score[1] || filt_score[1] == 0)))
                {
                    vDocIds[i] = -1;
                }
            }
